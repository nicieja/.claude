...
- `/writing-style <text>` — rewrite the pasted text.
- `/writing-style <file path>` — read the file and rewrite it. **Output to chat; never overwrite the source** unless explicitly told to save.
- `/writing-style` (bare) — rewrite the draft under discussion in the current conversation. If there's no obvious draft, ask once: "What should I rewrite?"
- Add a steer in plain words: `/writing-style --light ...` or "light touch" (fluency + gentle rhythm only), "go further" / "make it sing" (full voice), "keep my paragraph breaks," "this is non-fiction," etc. Default intensity = **match the source** (see Step 2).
...
Read `~/.claude/skills/writing-style/voice-guide.md` in full **before rewriting**. It is the spec. Do not rewrite from memory of it — load it every run, because it's the thing the author tunes over time.
