...
- They want the prose re-voiced into their own literary voice, or non-native fluency fixed → `/writing-style`.
...
- Compose with `/writing-style`: **deslop first** (cut the empty, the over-dense, the noise comments), **then voice** (re-voice what remains).
