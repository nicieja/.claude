...
- Read the schema of the app that defines each table you'll touch (`db/schema.rb`, the location named in `~/.claude/context/<project>/stack.md`, or discovered via `**/db/schema.rb`). Read the actual `create_table` block. Do not guess column names from model code alone.
...
Generate one script that produces the smallest evidence from Step 0, in the dialect of the channel from Step 0.5: SQL for a Metabase-style query MCP, ActiveRecord/Ruby for a hand-run Rails console, or whatever flavor stack.md names. The read-only rules below apply verbatim in every dialect.
...
- **Ruby string interpolation can't contain escaped double-quotes**: `"#{Foo.where(\"col = ?\", x).count}"` is a parse error. Ruby treats the inner `\"` as the end of the outer string. Hoist:
  ```ruby
  q = Foo.where("col = ?", x)
  puts "count: #{q.count}"
  ```
...
- For early exits, use `raise "descriptive message"`. Never use `next`, `break`, or `return`, which are syntax errors inside `begin/rescue` in Rails console
- Copy-paste ready, without `<FILL_IN>` placeholders or setup instructions beyond "paste this"

For raw SQL (psql / `EXPLAIN`), the same read-only rules apply. Wrap multi-statement scripts in `BEGIN READ ONLY; ... COMMIT;` if you want extra safety.
...
Execute the query yourself through the confirmed MCP tool and treat its result as the output. Show the user the query you ran alongside the verdict, because the query is part of the evidence.
...
Wait for the output. Do not proceed without it. If the user signals verification isn't available (no console access, a locked-down prod, or "skip this one"), return `Inconclusive` immediately with the reason and end the skill.
...
- **`Inconclusive — <reason>`**: the output is ambiguous, errored, or doesn't address the claim cleanly. State what would resolve it: a more targeted query or a different table, or the missing context.
