...
- Use the active voice. State the agent of each action.
...
- Keep the same content. Do not add new claims, and do not remove a caveat that changes the meaning. If the simplification would lose a nuance, keep the nuance and add a second sentence for it.
...
- Keep the file's own voice where STE allows it. The goal is a clearer version of the same document.
- After you write the file, report in one or two sentences how many paragraphs you changed. Also say where the rewrite lost a nuance, if any.
